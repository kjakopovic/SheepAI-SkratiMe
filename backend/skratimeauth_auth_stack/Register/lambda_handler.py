...
def lambda_handler(event, context):
    ...
    notion_link = event_body.get("notion_link", "")
    user_interests = event_body.get("user_interests", [])
    name = event_body.get("name", "")

    if not isinstance(user_interests, list) or not all(
        isinstance(item, str) for item in user_interests
    ):
        return {
            "statusCode": 400,
            "headers": HEADERS,
            "body": json.dumps({"message": "user_interests must be a list of strings"}),
        }

    logger.info(
        "Processing registration", extra={"email": email, "has_name": bool(name)}
    )
    ...
    return register_user(email, password, name, notion_link, user_interests)


def register_user(email, password, name, notion_link, user_interests):
    try:
        ...
        if notion_link:
            user_attributes.append({"Name": "custom:notion_link", "Value": notion_link})

        if user_interests:
            user_attributes.append(
                {"Name": "custom:user_interests", "Value": json.dumps(user_interests)}
            )
        ...
        logger.exception(
            "Unexpected error during registration", extra={"error": str(e)}
        )
        return {
            "statusCode": 500,
            "headers": HEADERS,
